...
namespace Shop.API.Controllers
{
    public class InventoryController : BaseApiController
    {
        ...
        {
            var result = await _inventoryFacade.GetInventoryById(inventoryId);
            return QueryResult(result);
        }

        [HttpGet]
        public async Task<ApiResult<InventoryFilterResult>> GetInventoryByFilter
            ([FromQuery] InventoryFilterParam filterParams)
        {
            var result = await _inventoryFacade.GetInventoryByFilter(filterParams);
            return QueryResult(result);
        }
    }
}
